...
fit_new_dim <- function(mod, obs){
  ...
  var_h6<-array(numeric(),dim=c(dim1[1],dim1[2]))
  
  var_h3<-mod[1:(nrow(mod)-1),1:(ncol(mod)-1)]
  
  var_h4<-change_dim(var_h3,dim_output = c(nrow(var_h3)*fact1, ncol(var_h3)*fact2))
  dimn<-dim(var_h4)
  
  odd_lon_var_h2<-matrix(rep(mod[ ,ncol(mod)],each=fact1)[1:(dim1[1]-1)],nrow=(dim1[1]-1))
  odd_lat_var_h2<-matrix(rep(mod[nrow(mod), ],each=fact2)[1:dim1[2]],nrow = 1)
  
  var_h5<-cbind(var_h4,odd_lon_var_h2)
  var_h6[ , ]<-rbind(var_h5,odd_lat_var_h2)
