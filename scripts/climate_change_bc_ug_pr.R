...
for (p in 1:2){ ## periods
  ...
  for (s in 1:4){ ## scen
    ## future
    var_bc1<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/pr_Amon_',mod[1],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
    var_bc2<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/pr_Amon_',mod[2],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
    var_bc3<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/pr_Amon_',mod[3],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
    var_bc4<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/pr_Amon_',mod[4],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
    var_bc5<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/pr_Amon_',mod[5],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
  ...
  l<-c(-2,2)
  b<-seq(-2,2,0.5)
  tit =paste0('Uganda change in precipitation in'," ",pd1[p])
  unit="mm day-1"
  ...
  ggsave(plot = gg, 
         filename = paste0(dir,'plots/climate_change/bias_cor/Ugan_pr_climate_',pd1[p],'.png'), 
