...
for (n in 1:6){ ## models
  for (k in 1:2){ ## 2030 or 2050?
    for (j in 1:4){ ## scenario
      ...
      ### 1. obs ref
      var_obs<-nc_open(paste0(dir,'observed_baseline/agmerra/srad_daily_ts_agmerra_1991_2010_ugan_inv.nc'))
      ...
      var_f<-nc_open(paste0(dir,'cc_raw/regrid/future/Uganda/',inp[k],'/monmean/rsds_Amon_',mod[n],'_ssp',sc[j],'_','r1i1p1f1_monmean_Ugan_',prd[k],'.nc'))
      ...
      ## History
      ## 4. historical mean
      var_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monmean/rsds_Amon_',mod[n],'_historical_r1i1p1f1_Ugan_monmean_199101-201012.nc'))
      ...
      saveRDS(bc_rad, paste0(dir,'cc_bc/Uganda/',ti[k],'/rsds_Amon_',mod[n],'_ssp',sc[j],'_',
