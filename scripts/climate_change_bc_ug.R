...
for (p in 1:2){ ## periods
  for (v in 1:3){ ## var
    ...
    for (s in 1:4){ ## scen
      ## future
      var_bc1<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[1],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
      var_bc2<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[2],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
      var_bc3<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[3],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
      var_bc4<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[4],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
      var_bc5<-nc_open(paste0(dir,'cc_bc/Uganda/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[5],'_ssp',sc[s],'_r1i1p1f1_Ugan_bc_',pd1[p],'.climate.nc'))
    ...
    if(v==1){
      ...
      l<-c(-1,6)
      b<-seq(-1,6,1)
      tit=paste0('Uganda change in Tmax in'," ",pd1[p])
      unit="0C"
    } else if(v==2){
      cols=c('#d0d1e6','#ffffb2','#fed976','#feb24c','#fd8d3c','#f03b20','#bd0026')
      l<-c(-1,6)
      b<-seq(-1,6,1)
      tit=paste0('Uganda change in Tmin in'," ",pd1[p]) 
      unit="0C"
    } else {
      ...
      tit=paste0('Uganda change in Srad in'," ",pd1[p]) 
      unit="MJm-2"
    }
    ...
    ggsave(plot = gg, 
           filename = paste0(dir,'plots/climate_change/bias_cor/Ugan_',var[v],'_climate_',pd1[p],'.png'), 
