...
dir1<-paste0(dir,'data_for_dssat_eaf/dssat_input/Tanzania/')
dir2<-paste0(dir,'data_for_dssat_eaf/cc_raw/scripts/')
dir_dssat <- paste0(dir, "dssat_test1/uganda_input/dssat-csm-original/Data/")

functions_to_run <- list.files(paste0(dir2,'functions_tz/'), full.names = T)
  ...
  #for (p in 1:5){ ## model
    #for (n in 1:4){ ## scenario
      x<-readRDS(paste0(dir1,year[k],'/',mod[p],'_ssp',sc[n],'_r1i1p1f1_',period[k],'.RDS'))
       dimn<-dim(x)
	   x<-x[420:dimn[1], ] ## broken at 420 point
      ...
      for (i in 1:length(x$Code)){
        dir.create(file.path(paste0(dir,'dssat_outputs/tanzania/dssat_runs_tanzania_',mod[p],'_ssp_',sc[n],'_',year[k],'/'),x$Code[i]), recursive =T, showWarnings = F)
        out_dir<-paste0(dir,'dssat_outputs/tanzania/dssat_runs_tanzania_',mod[p],'_ssp_',sc[n],'_',year[k],'/',x$Code[i],'/')
        setwd(out_dir)
        ...
        file.copy(paste0(dir2,'original_files_tz/BNGRO047.CUL'), out_dir)
        file.copy(paste0(dir2,'original_files_tz/BNGRO047.SPE'), out_dir)
        file.copy(paste0(dir2,'original_files_tz/BNGRO047.ECO'), out_dir)
		
		if(x$jday_end_avg[i] == 0) {
		 file.copy(paste0(dir2,'original_files_tz/TANZ9105.BNX'), out_dir)
		 } else {
		 file.copy(paste0(dir2,'original_files_tz/CCPA9105.BNX'), out_dir)
		 }
