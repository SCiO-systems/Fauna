    ...
    for (j in 1:4){ ## scenario
      for (i in 1:3){ ### variables tmx, tmin and srad 
        ...
        ## to make lat, lon consistent with that of model
          if (n==2){
           var_obs2<-var_obs1[7:31,49:61, ] 
          } else if(n==3){
           var_obs2<-var_obs1[1:28,40:67, ]
          } else if(n==5){
           var_obs2<-var_obs1[7:27,39:69, ]
          } else if(n==6){
           var_obs2<-var_obs1[1:25,43:67, ]
          } else {
           var_obs2<-var_obs1[2:27,39:69, ] 
          }
          
        ## future
        ## 2. raw mean
        if(n==3 | n==5){
          var_f<-nc_open(paste0(dir,'cc_raw/regrid/future/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_',
                                'r1i1p1f1_re_gr_Ugan_monmean_',prd[k],'.nc'))
        } else if (n==4){
          var_f<-nc_open(paste0(dir,'cc_raw/regrid/future/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_',
                                'r1i1p1f1_re_gr1_Ugan_monmean_',prd[k],'.nc'))
        } else {
          var_f<-nc_open(paste0(dir,'cc_raw/regrid/future/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_',
                                'r1i1p1f1_re_gn_Ugan_monmean_',prd[k],'.nc'))
        }
        ...
        ln2<-which.min(abs(mod_ln - 35.125))
        ...
        ## 3. std raw
        if(n==3 | n==5){
          var_std_f<-nc_open(paste0(dir, 'cc_raw/regrid/future/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_re_gr_Ugan_monstd_',prd[k],'.nc'))
        } else if (n==4){
          var_std_f<-nc_open(paste0(dir, 'cc_raw/regrid/future/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_re_gr1_Ugan_monstd_',prd[k],'.nc'))
        } else {
          var_std_f<-nc_open(paste0(dir, 'cc_raw/regrid/future/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_re_gn_Ugan_monstd_',prd[k],'.nc'))
        }
        ...
        ## 4. historical mean
        if(n==3 | n==5){
          var_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monmean/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_re_gr_Ugan_monmean_185001-201412.nc'))
        } else if (n==4){
          var_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monmean/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_re_gr1_Ugan_monmean_185001-201412.nc'))
        } else {
          var_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monmean/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_re_gn_Ugan_monmean_185001-201412.nc'))
        }
        ...
        ## 5. std histo
        if(n==3 | n==5){
          var_std_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monstd/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_re_gr_Ugan_monstd_185001-201412.nc'))
        } else if (n==4){
          var_std_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monstd/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_re_gr1_Ugan_monstd_185001-201412.nc'))
        } else {
          var_std_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monstd/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_re_gn_Ugan_monstd_185001-201412.nc'))
        }
        ...
        ## save file
        saveRDS(bias_cor_var, paste0(dir,'cc_bc/Uganda/',ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_',
                                     'r1i1p1f1_re_gn_Ugan_bc_',prd[k],'.nc'))
      }
    }
  }
}
