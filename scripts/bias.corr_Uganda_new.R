  ...
  for (k in 1:2){ ## 2030 or 2060?
    for (j in 1:4){ ## scenario
      for (i in 1:2){ ### variables tmx, tmin and srad 
        ...
        ### 1. obs ref
        var_obs<-nc_open(paste0(dir,'observed_baseline/agmerra/',var1[i],'_daily_ts_agmerra_1991_2010_ugan_inv.nc'))
        ...
        if(n==2){
          var_obs2<-var_obs1[2:35,2:38, ] ## n=2
        } else if(n==4){
          var_obs2<-var_obs1[1:31,2:37, ]
        } else if(n==5){
          var_obs2<-var_obs1[5:35,5:35, ]
        } else {
          var_obs2<-var_obs1[1:36,2:37, ] ## n=1 ## n=3
        }
        
        ## future
        ## 2. raw mean
        var_f<-nc_open(paste0(dir,'cc_raw/regrid/future/Uganda/',inp[k],'/monmean/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_',
        ...
        ln1<-which.min(abs(mod_ln - 27.875))
        ln2<-which.min(abs(mod_ln - 36.125))
        lt1<-which.min(abs(mod_lt - -3.625))
        lt2<-which.min(abs(mod_lt - 5.125))
        
        var_f2<-var_f1[ln1:ln2,lt1:lt2, ] #to make lon consistent with the model
        
        ## 3. std raw
        var_std_f<-nc_open(paste0(dir, 'cc_raw/regrid/future/Uganda/',inp[k],'/monstd/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_monstd_Ugan_',prd[k],'.nc'))
        var_std_f1<-ncvar_get(var_std_f, var[i])
        var_std_f2<-var_std_f1[ln1:ln2,lt1:lt2, ] #to make lon consistent with the model
        
        ## History
        ## 4. historical mean
        var_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monmean/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_Ugan_monmean_199101-201012.nc'))
        var_h1<-ncvar_get(var_h, var[i])-273
        var_h2<-var_h1[ln1:ln2,lt1:lt2, ] #to make lon consistent with the model
        
        ## 5. std histo
        var_std_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Uganda/monstd/',var[i],'_Amon_',mod[n],'_historical_r1i1p1f1_Ugan_monstd_199101-201012.nc'))
