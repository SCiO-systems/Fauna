...
for (k in 1:2){ ## periods
  for (j in 1:4){ ## scenarios
    ...
    # data<-brick(ch_all1, xmn=30.125, xmx=34.625, ymn=-2.875, ymx=4.625, 
    ...
    data<-brick(ch_all2, xmn=30.125, xmx=34.625, ymn=-2.875, ymx=4.625, 
                       crs=NA, transpose=TRUE)
    
    ln=seq(from=30.125,to=34.625,by=0.25)
    lt=seq(from=-2.875,to=4.625,by=0.25)
   ...
   ## lable for models
    ...
    gg <- ggplot(vls)  +
      ...
      labs(title = paste0('Uganda (BC - Raw)'," ",pd[k]," ",sc1[j]), fill = '0C/MJm-2day-1', x = 'Longitude', y = 'Latitude') +  
          theme(legend.position = 'bottom', 
            ...
            legend.key.width = unit(5, 'line'))
               # Saving the plot
    ggsave(plot = gg, 
           filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/diff_bc-raw/diff_others/ggplot/ugan_ch_others_bc_raw_',tim[k],'_',sc1[j],'.png'), 
