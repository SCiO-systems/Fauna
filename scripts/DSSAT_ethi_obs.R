...
CNTR<-c('Uganda', 'Tanzania', 'Ethiopia')
cntr<-c('uganda', 'tanzania', 'ethiopia')
cnt<-c('ugan','tanz','ethi')

n=3
dir<-'//dapadfs/workspace_cluster_12/AVISA/'
dir1<-paste0(dir,'data_for_dssat_eaf/dssat_input/',CNTR[n],'/')
...
## 1. obs
x<-readRDS(paste0(dir1,'obs/agMeChrips_',cnt[n],'_lat4.6_8.9_1991_2010.RDS'))
...
for (i in 1:length(x$Code)){
  dir.create(file.path(paste0(dir,'dssat_outputs/',cntr[n],'/agMeChrips_',cnt[n],'_1991_2010/'),x$Code[i]), recursive =T, showWarnings = F)
  out_dir<-paste0(dir,'dssat_outputs/',cntr[n],'/agMeChrips_',cnt[n],'_1991_2010/',x$Code[i],'/')
  setwd(out_dir)
  
  ## 1. weather
  make_wth(data = x$climate[[i]], lat = x$lat[[i]], out_dir = out_dir,
           lon = x$lon[[i]],  co2 = -99, name_xfile_climate = 'UGAND001')
  ...
  ## Simulation control: Actual CO2; Mauna Loa, Hawaii (Keeling Curve)measured data Mauna Lauwa
  ...
  rds_dir<-paste0(dir,'dssat_outputs/',cntr[n],'/agMeChrips_',cnt[n],'_1991_2010/')
  ...
  saveRDS(lis[[l]],paste0(lis1[l],'_',cntr[n],'_agMeChrips_1991_2010.RDS'))
  }
