...
for (j in 1:4){ ## scenarios
  ...
  lon=seq(from=35.1,to=46.35,by=0.05)
  lat=seq(from=4.6,to=15.1,by=0.05)
  
  ## for tmx
   PlotLayout(PlotEquiMap, c('lat','lon'), var_mod_all2[1:12, , ],lon=lon, lat=lat, nrow=6, filled.continents=FALSE, units = "mm day-1",
              toptitle=paste0('Ethiopia precipitation annual average'," ",pd[k]," ",sc1[j]), row_titles = mod, col_titles = c('Raw', 'Bias corrected'),
              bar_limits = c(0,9), width=8, height=20, cols=c('#ffffff','#f7fcfd','#e5f5f9','#ccece6','#99d8c9','#66c2a4','#41ae76','#238b45','#006d2c'),
              col_sup='#00441b',  bar_scale=0.3, intylat = 1, intxlon = 1, colNA="white",
              title_scale=0.6, extra_margin = c(1,2,1,2), fileout = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/precip/ethi_precip_raw_bc_',tim[k],'_',sc1[j],'.png'))
   
  
  #PlotLayout(PlotEquiMap, c('lat','lon'), ch_all2[1:6, , ],lon=lon, lat=lat, nrow=6, filled.continents=FALSE, units = "mm day-1",
            # toptitle=paste0('Ethiopia difference between BC and Raw'," ",pd[k]," ",sc1[j]), row_titles = mod, 
             ...
             #title_scale=0.6, extra_margin = c(1,2,1,2), fileout = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/precip/ethi_ch_precip_bc_raw_',tim[k],'_',sc1[j],'.png'))
