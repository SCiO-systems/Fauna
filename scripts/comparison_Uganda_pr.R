...
for (k in 1:2){ ## periods
  for (j in 1:4){ ## scenarios
    ...
    ## to make lat/lon same for every model## select lat (-2.125:3.875) ,lon=30.125:34.625
    var_raw1_an<-var_raw1_an
    var_bc1_an<-var_bc1_an
    ch1_an<-var_bc1_an-var_raw1_an
    
    var_raw2_an<-var_raw2_an[,1:121]
    var_bc2_an<-var_bc2_an[,1:121]  
    ch2_an<-var_bc2_an-var_raw2_an
    
    var_raw3_an<-var_raw3_an
    var_bc3_an<-var_bc3_an
    ch3_an<- var_bc3_an-var_raw3_an
    
    var_raw4_an<-var_raw4_an
    var_bc4_an<-var_bc4_an
    ch4_an<-var_bc4_an-var_raw4_an
    
    var_raw5_an<-var_raw5_an[,1:121]
    var_bc5_an<-var_bc5_an[,1:121]
    ch5_an<-var_bc5_an-var_raw5_an
    
    var_raw6_an<-var_raw6_an
    var_bc6_an<-var_bc6_an
    ...
    #mps <- shapefile('D:/OneDrive - CGIAR/Desktop/UGA_adm/UGA_adm0.shp')
    ...
    tit =paste0('Uganda precipitation annual average'," ",pd[k]," ",sc1[j])
    ...
    ggsave(plot = gg, 
           filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/precip/precip_gg/ugan_precip_raw_bc_',tim[k],'_',sc1[j],'.png'), 
