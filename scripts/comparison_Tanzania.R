...
for (k in 1:2){ ## periods
  for (i in 1:3){ ## varibles
    for (j in 1:4){ ## scenarios
      ...
      data<-brick(var_mod_all2, xmn=30.125, xmx=40.125, ymn=-11.125, ymx=-1.375, 
      ...
      if(i==1){
        x<-c(20,40)
        y<-seq(20,40,2)
        tit=paste0('Tanzania Tmax annual average'," ",pd[k]," ",sc1[j])
        unit="0C"
      } else if(i==2){
        x<-c(8,28)
        y<-seq(8,28,2)
        tit=paste0('Tanzania Tmin annual average'," ",pd[k]," ",sc1[j])
        unit="0C"
      } else {
        x<-c(16,24)
        y<-seq(16,24,1)
        tit=paste0('Tanzania Srad annual average'," ",pd[k]," ",sc1[j])
        unit="MJm-2"
      }
      ...
      ggsave(plot = gg, 
             filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/others/ggplot/tz_',varbl[i],'_raw_bc_',tim[k],'_',sc1[j],'.png'), 
