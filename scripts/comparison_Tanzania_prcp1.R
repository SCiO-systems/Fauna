...
for (k in 1:2){ ## periods
  for (j in 1:4){ ## scenarios
    ...
    data<-brick(var_mod_all2, xmn=30.1, xmx=39.1, ymn=-10.35, ymx=-1.35,
    ...
    cols=c('#f7fcfd','#e5f5f9','#ccece6','#99d8c9','#66c2a4','#41ae76','#238b45','#006d2c')
    tit=paste0('Tanzania precipitation annual average'," ",pd[k]," ",sc1[j])
    
    gg <- ggplot(vls)  +
      geom_tile(aes(x = x, y =  y, fill = value)) +
      facet_wrap(~ mdl_name, nrow=6, ncol=2) +
      scale_fill_gradientn(colours = cols, guide = "colourbar",
                           na.value = 'white', limits = c(0,8), breaks =seq(0,8,1) ) +
      geom_polygon(data = mps, aes(x=long, y = lat, group = group),color = 'grey', fill='NA') +
      theme_bw() +
      scale_x_continuous(breaks = seq(30,40,by=2)) + 
      scale_y_continuous(breaks=seq(-10,-1,by=3)) +
      coord_equal(xlim = c(29.5,40.3), ylim = c(-10, -1)) +
    ...
    ggsave(plot = gg, 
           filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/precip/ggplot/tanz_precip_raw_bc_',tim[k],'_',sc1[j],'.png'), 
