...
Product_id<- read.csv('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/cc_raw/scripts/proces_ID.csv')
k=product_id[2]
p=product_id[3]
n=product_id[4]
...
#for (k in 1:2) {
  #for (p in 1:5) { ## model
    #for (n in 1:4) { ## scenario
      x<-readRDS(paste0(dir1,year[k],'/',p,'_ssp',sc[n],'_r1i1p1f1_',period[k],'.RDS'))
