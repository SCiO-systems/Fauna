...
for (p in 1:2){ ## periods
  bc1<-array(numeric(),c(276,276,4))
  bc2<-array(numeric(),c(271,286,4))
  bc3<-array(numeric(),c(276,276,4))
  bc4<-array(numeric(),c(251,276,4))
    bc5<-array(numeric(),c(276,276,4))
  
  for (s in 1:4){ ## scen
    ## future
    var_bc1<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/pr_Amon_',mod[1],'_ssp',sc[s],'_r1i1p1f1_Tanz_bc_',pd1[p],'.climate.nc'))
    var_bc2<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/pr_Amon_',mod[2],'_ssp',sc[s],'_r1i1p1f1_Tanz_bc_',pd1[p],'.climate.nc'))
    var_bc3<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/pr_Amon_',mod[3],'_ssp',sc[s],'_r1i1p1f1_Tanz_bc_',pd1[p],'.climate.nc'))
    var_bc4<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/pr_Amon_',mod[4],'_ssp',sc[s],'_r1i1p1f1_Tanz_bc_',pd1[p],'.climate.nc'))
    var_bc5<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/pr_Amon_',mod[5],'_ssp',sc[s],'_r1i1p1f1_Tanz_bc_',pd1[p],'.climate.nc'))
  ...
  ## historical
  var_raw1<-nc_open(paste0(dir_hist,'Tanzania/timmean/pr_Amon_',mod[1],'_historical_r1i1p1f1_Tanz_climate_199101-201012.nc'))
  var_raw2<-nc_open(paste0(dir_hist,'Tanzania/timmean/pr_Amon_',mod[2],'_historical_r1i1p1f1_Tanz_climate_199101-201012.nc'))
  var_raw3<-nc_open(paste0(dir_hist,'Tanzania/timmean/pr_Amon_',mod[3],'_historical_r1i1p1f1_Tanz_climate_199101-201012.nc'))
  var_raw4<-nc_open(paste0(dir_hist,'Tanzania/timmean/pr_Amon_',mod[4],'_historical_r1i1p1f1_Tanz_climate_199101-201012.nc'))
  var_raw5<-nc_open(paste0(dir_hist,'Tanzania/timmean/pr_Amon_',mod[5],'_historical_r1i1p1f1_Tanz_climate_199101-201012.nc'))
  ...
  ## to aind all, take the min long and lat of IPSL (27.875:40.125, -11.875:1.375)
  fbc1<-bc1[6:251,11:276,];fbc2<-bc2[1:246,16:281,];fbc3<-bc3[6:251,11:276,]
  fbc4<-bc4[6:251,11:276,]; fbc5<-bc5[6:251,11:276,]
  
  fraw1_new<-raw1_new[6:251,11:276];fraw2_new<-raw2_new[1:246,16:281]; fraw3_new<-raw3_new[6:251,11:276]
  fraw4_new<-raw4_new[6:251,11:276]; fraw5_new<-raw5_new[6:251,11:276]
  ...
  ## 1.1. Obs
  file<-nc_open(paste0(dir,'observed_baseline/chirps/nc/proc/chirps-v2.0.1991_2010.days_p05_climate.tanz.nc')) 
  ...
  ggsave(plot = gg, 
         filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/climate/bias_cor/Tanz_pr_climate_',pd1[p],'.png'), 
