...
for (k in 1:2){ ## periods
  for (j in 1:4){ ## scenarios
    cha_all<-array(numeric(),c(50,49,6,3))
    for (i in 1:3){ ## varibles
      if (k==1){
        var_raw1<-nc_open(paste0(dir_raw,tim[k],'/',var[i],'_Amon_',mod[1],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw2<-nc_open(paste0(dir_raw,tim[k],'/',var[i],'_Amon_',mod[2],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw3<-nc_open(paste0(dir_raw,tim[k],'/',var[i],'_Amon_',mod[3],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw4<-nc_open(paste0(dir_raw,tim[k],'/',var[i],'_Amon_',mod[4],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw5<-nc_open(paste0(dir_raw,tim[k],'/',var[i],'_Amon_',mod[5],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw6<-nc_open(paste0(dir_raw,tim[k],'/',var[i],'_Amon_',mod[6],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
      } else {
        var_raw1<-nc_open(paste0(dir_raw,tim1[k],'/',var[i],'_Amon_',mod[1],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw2<-nc_open(paste0(dir_raw,tim1[k],'/',var[i],'_Amon_',mod[2],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw3<-nc_open(paste0(dir_raw,tim1[k],'/',var[i],'_Amon_',mod[3],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw4<-nc_open(paste0(dir_raw,tim1[k],'/',var[i],'_Amon_',mod[4],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw5<-nc_open(paste0(dir_raw,tim1[k],'/',var[i],'_Amon_',mod[5],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
        var_raw6<-nc_open(paste0(dir_raw,tim1[k],'/',var[i],'_Amon_',mod[6],'_',sc1[j],'_r1i1p1f1_monmean_Ethi_',pd[k],'.nc'))
      }
      
      
      if (i==3){
        var_raw1<-ncvar_get(var_raw1,var[i])*0.0864
        var_raw1<-var_raw1[1:12, , ]
        var_raw2<-ncvar_get(var_raw2,var[i])*0.0864
        var_raw2<-var_raw2[2:20,1:18, ]
        var_raw3<-ncvar_get(var_raw3,var[i])*0.0864
        var_raw3<-var_raw3[1:12, , ]
        var_raw4<-ncvar_get(var_raw4,var[i])*0.0864
        var_raw5<-ncvar_get(var_raw5,var[i])*0.0864
        var_raw6<-ncvar_get(var_raw6,var[i])*0.0864
        var_raw6<-var_raw6[1:12, , ]
      } else {
      ...
      var_bc1<-nc_open(paste0(dir_bc,tim[k],'/nc/mnmean/',var[i],'_Amon_',mod[1],'_',sc1[j],'_r1i1p1f1_Ethi_bc_',pd[k],'.mnmean.nc'))
      var_bc2<-nc_open(paste0(dir_bc,tim[k],'/nc/mnmean/',var[i],'_Amon_',mod[2],'_',sc1[j],'_r1i1p1f1_Ethi_bc_',pd[k],'.mnmean.nc'))
      var_bc3<-nc_open(paste0(dir_bc,tim[k],'/nc/mnmean/',var[i],'_Amon_',mod[3],'_',sc1[j],'_r1i1p1f1_Ethi_bc_',pd[k],'.mnmean.nc'))
      var_bc4<-nc_open(paste0(dir_bc,tim[k],'/nc/mnmean/',var[i],'_Amon_',mod[4],'_',sc1[j],'_r1i1p1f1_Ethi_bc_',pd[k],'.mnmean.nc'))
      var_bc5<-nc_open(paste0(dir_bc,tim[k],'/nc/mnmean/',var[i],'_Amon_',mod[5],'_',sc1[j],'_r1i1p1f1_Ethi_bc_',pd[k],'.mnmean.nc'))
      var_bc6<-nc_open(paste0(dir_bc,tim[k],'/nc/mnmean/',var[i],'_Amon_',mod[6],'_',sc1[j],'_r1i1p1f1_Ethi_bc_',pd[k],'.mnmean.nc'))
      ...
      var_raw1_an<-var_raw1_an[6:55,3:51]
      var_bc1_an<-var_bc1_an[6:55,3:51]
      ch1_an<-var_bc1_an-var_raw1_an
      
      var_raw2_an<-var_raw2_an[6:55,4:52]
      var_bc2_an<-var_bc2_an[6:55,4:52]  
      ch2_an<-var_bc2_an-var_raw2_an
      
      var_raw3_an<-var_raw3_an[6:55,3:51]
      var_bc3_an<-var_bc3_an[6:55,3:51]
      ch3_an<- var_bc3_an-var_raw3_an
      
      var_raw4_an<-var_raw4_an[1:50,3:51]
      var_bc4_an<-var_bc4_an[1:50,3:51]
      ch4_an<-var_bc4_an-var_raw4_an
      
      var_raw5_an<-var_raw5_an[9:58,1:49]
      var_bc5_an<-var_bc5_an[9:58,1:49]
      ch5_an<-var_bc5_an-var_raw5_an
      
      var_raw6_an<-var_raw6_an[6:55,3:51]
      var_bc6_an<-var_bc6_an[6:55,3:51]
    ...
    # data<-brick(ch_all1, xmn=35.125, xmx=47.375, ymn=3.125, ymx=15.125, 
    ...
    # lon=seq(from=35.125,to=47.375,by=0.25)
    # lat=seq(from=3.125,to=15.125,by=0.25)
    ...
    data<-brick(ch_all2, xmn=35.125, xmx=47.375, ymn=3.125, ymx=15.125, 
    ...
    gg <- ggplot(vls)  +
      ...
      labs(title = paste0('Ethiopia (BC - Raw)'," ",pd[k]," ",sc1[j]), fill = '0C/MJm-2day-1', x = 'Longitude', y = 'Latitude') +  
    ...
    # Saving the plot
    ggsave(plot = gg, 
           filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/diff_bc-raw/diff_others/ggplot/eth_ch_others_bc_raw_',tim[k],'_',sc1[j],'.png'), 
