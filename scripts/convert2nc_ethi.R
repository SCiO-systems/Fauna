...
var_obs<-nc_open(paste0(dir,'observed_baseline/agmerra/tmax_daily_ts_agmerra_1991_2010_eth_inv.nc'))
...
n=5
  ...
  i=1
    #for (i in 1:3){ ## var
      for (j in 1:4){ #
          vari<-readRDS(paste0(dir,'cc_bc/Ethiopia/',pd[p],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_Ethi_bc_',trange[p],'.RDS'))       
          dimn<-dim(vari)
        if(i==1 | i==2){
        ...
        if(n==5){
          lon<-obs_ln[6:56]; lat<-obs_lt[8:56]
        } else if (n==2) {
          lon<-obs_ln[3:66]; lat<-obs_lt[5:56]
        } else {
          lon<-obs_ln[4:64]; lat<-obs_lt[6:56]
        }
        ...
        ArrayToNc(list(vari, lon,lat), paste0(dir,'cc_bc/Ethiopia/',pd[p],'/nc/',
                                              var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_Ethi_bc_',trange[p],'.nc'))  
      }
    }
  #}
#}
