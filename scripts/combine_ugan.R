...
library(ncdf4)
library(abind)
dir<-'//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/cc_bc/Uganda/'
...
for (n in 1:5){ ## models
  for (k in 1:2){ ## 2030 or 2060?
    for (j in 1:4){ ## scenario
      ...
      pr<-readRDS(paste0(dir,ti[k],'/','pr_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_Ugan_bc_',prd[k],'.RDS'))
      
      dimen<-dim(pr)
      others<-array(numeric(), c(dimen[1],dimen[2],dimen[3],3))
      for (i in 1:3){ ### variables tmx, tmin and srad 
        
        dd<-readRDS(paste0(dir,ti[k],'/',var[i],'_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_Ugan_bc_',prd[k],'.RDS'))
        
        if (n==2){
          dd<-dd[,7:37,] ## 2
        } else {
          dd<-dd[,6:36,] ## all others
        }
      ...
      ## create directory
      dir.create(file.path(paste0(dir,ti[k]), showWarnings = FALSE))
      
      saveRDS(all,paste0(dir,ti[k],'/','all_Amon_',mod[n],'_ssp',sc[j],'_r1i1p1f1_Ugan_bc_',prd[k],'.RDS'))
      rm(all)
    }
  }
}
