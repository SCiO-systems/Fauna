...
## 1.2. Model future. since Tanzania has smaller area, coarser resolution models like CanESM5
...
for (n in 1:6){ ## models
  for (k in 1:2){ ## 2030 or 2060?
    for (j in 1:4){ ## scenario
      ...
      var_f<-nc_open(paste0(dir,'cc_raw/regrid/future/Tanzania/',inp[k],'/monmean/pr_Amon_',mod[n],'_ssp',sc[j],'_',
                            'r1i1p1f1_monmean_Tanz_',prd[k],'.nc'))
      ...
      if(n==1 | n==3 | n==6){
        var_obs2<-obs[17:292,33:308, ] #n=1 #t2
      } else if (n==2){
        var_obs2<-obs[22:292,28:313, ] #n=2
      } else if (n==4){
        var_obs2<-obs[17:267,33:308, ] # n=4
      } else {
        var_obs2<-obs[7:307,43:313, ] # n=5
      }
        ...
        if (n==5){
          ln1<-which.min(abs(mod_ln - 27.1))
          ln2<-which.min(abs(mod_ln - 42.1))
        }
      ...
      var_h<-nc_open(paste0(dir,'cc_raw/regrid/historical/Tanzania/monmean/',"pr",'_Amon_',mod[n],
                            '_historical_r1i1p1f1_Tanz_monmean_199101-201012.nc'))
      ...
      ## save file
      ## directory create
      dir.create(file.path(paste0(dir,'cc_bc/Tanzania/all/',ti[k],'/all_Tanz/precip/'), showWarnings = FALSE))
      
      saveRDS(bc_prec, paste0(dir,'cc_bc/Tanzania/all/',ti[k],'/all_Tanz/precip/pr_Amon_',mod[n],'_ssp',sc[j],'_',
                              'r1i1p1f1_Tanz_bc_all_',prd[k],'.RDS'))
      rm(bc_prec)
