...
for (p in 1:2){ ## periods
  for (v in 1:3){ ## var
    bc1<-array(numeric(),c(56,56,4))
    bc2<-array(numeric(),c(55,58,4))
    bc3<-array(numeric(),c(56,56,4))
    bc4<-array(numeric(),c(51,56,4))
    bc5<-array(numeric(),c(56,56,4))
    
    for (s in 1:4){ ## scen
      ## future
      var_bc1<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[1],'_ssp',sc[s],'_r1i1p1f1_tanz_bc_',pd1[p],'.climate.nc'))
      var_bc2<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[2],'_ssp',sc[s],'_r1i1p1f1_tanz_bc_',pd1[p],'.climate.nc'))
      var_bc3<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[3],'_ssp',sc[s],'_r1i1p1f1_tanz_bc_',pd1[p],'.climate.nc'))
      var_bc4<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[4],'_ssp',sc[s],'_r1i1p1f1_tanz_bc_',pd1[p],'.climate.nc'))
      var_bc5<-nc_open(paste0(dir_fut,'Tanzania/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[5],'_ssp',sc[s],'_r1i1p1f1_tanz_bc_',pd1[p],'.climate.nc'))
    ...
    ## historical
    var_raw1<-nc_open(paste0(dir_hist,'Tanzania/timmean/',var[v],'_Amon_',mod[1],'_historical_r1i1p1f1_tanz_climate_199101-201012.nc'))
    var_raw2<-nc_open(paste0(dir_hist,'Tanzania/timmean/',var[v],'_Amon_',mod[2],'_historical_r1i1p1f1_tanz_climate_199101-201012.nc'))
    var_raw3<-nc_open(paste0(dir_hist,'Tanzania/timmean/',var[v],'_Amon_',mod[3],'_historical_r1i1p1f1_tanz_climate_199101-201012.nc'))
    var_raw4<-nc_open(paste0(dir_hist,'Tanzania/timmean/',var[v],'_Amon_',mod[4],'_historical_r1i1p1f1_tanz_climate_199101-201012.nc'))
    var_raw5<-nc_open(paste0(dir_hist,'Tanzania/timmean/',var[v],'_Amon_',mod[5],'_historical_r1i1p1f1_tanz_climate_199101-201012.nc'))
    ...
    fbc1<-bc1[2:51,3:56,];fbc2<-bc2[1:50,4:57,];fbc3<-bc3[2:51,3:56,];fbc4<-bc4[2:51,3:56,]; fbc5<-bc5[2:51,3:56,]
    
    fraw1_new<-raw1_new[2:51,3:56];fraw2_new<-raw2_new[1:50,4:57]; fraw3_new<-raw3_new[2:51,3:56]; fraw4_new<-raw4_new[2:51,3:56]
     fraw5_new<-raw5_new[2:51,3:56]
    ## bind raw
    ...
    ### 1. obs ref
    var_obs<-nc_open(paste0(dir,'observed_baseline/agmerra/',var1[v],'_daily_ts_agmerra_1991_2010_tanz_inv_climate.nc'))
    ...
    data<-brick(df2, xmn=27.875, xmx=40.125, ymn=-11.875, ymx=1.375, 
    ...
    if(v==4){
      ...
      l<-c(0,12)
      b<-seq(0,12,1.5)
      tit =paste0('Tanzania precipitation in'," ",pd1[p])
      unit="mm day-1"
    } else if(v==1){
      cols=c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026')
      l<-c(12,52)
      b<-seq(12,52,5)
      tit=paste0('Tanzania Tmax in'," ",pd1[p])
      unit="0C"
    } else if(v==2){
      cols=c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026')
      l<-c(10,34)
      b<-seq(10,34,3)
      tit=paste0('Tanzania Tmin in'," ",pd1[p]) 
      unit="0C"
    } else {
      cols=c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026')
      l<-c(12,28)
      b<-seq(12,28,2)
      tit=paste0('Tanzania Srad in'," ",pd1[p]) 
      unit="MJm-2"
    }
    ...
    ggsave(plot = gg, 
           filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/climate/bias_cor/tanz_',var[v],'_climate_',pd1[p],'.png'), 
