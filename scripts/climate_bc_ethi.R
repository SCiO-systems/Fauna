...
for (p in 1:2){ ## periods
  for (v in 1:3){ ## var
    ...
    for (s in 1:4){ ## scen
      ## future
      var_bc1<-nc_open(paste0(dir_fut,'Ethiopia/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[1],'_ssp',sc[s],'_r1i1p1f1_ethi_bc_',pd1[p],'.climate.nc'))
      var_bc2<-nc_open(paste0(dir_fut,'Ethiopia/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[2],'_ssp',sc[s],'_r1i1p1f1_ethi_bc_',pd1[p],'.climate.nc'))
      var_bc3<-nc_open(paste0(dir_fut,'Ethiopia/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[3],'_ssp',sc[s],'_r1i1p1f1_ethi_bc_',pd1[p],'.climate.nc'))
      var_bc4<-nc_open(paste0(dir_fut,'Ethiopia/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[4],'_ssp',sc[s],'_r1i1p1f1_ethi_bc_',pd1[p],'.climate.nc'))
      var_bc5<-nc_open(paste0(dir_fut,'Ethiopia/',pd[p],'/nc/mnmean/',var[v],'_Amon_',mod[5],'_ssp',sc[s],'_r1i1p1f1_ethi_bc_',pd1[p],'.climate.nc'))
    ...
    ## historical
    var_raw1<-nc_open(paste0(dir_hist,'Ethiopia/timmean/',var[v],'_Amon_',mod[1],'_historical_r1i1p1f1_ethi_climate_199101-201012.nc'))
    var_raw2<-nc_open(paste0(dir_hist,'Ethiopia/timmean/',var[v],'_Amon_',mod[2],'_historical_r1i1p1f1_ethi_climate_199101-201012.nc'))
    var_raw3<-nc_open(paste0(dir_hist,'Ethiopia/timmean/',var[v],'_Amon_',mod[3],'_historical_r1i1p1f1_ethi_climate_199101-201012.nc'))
    var_raw4<-nc_open(paste0(dir_hist,'Ethiopia/timmean/',var[v],'_Amon_',mod[4],'_historical_r1i1p1f1_ethi_climate_199101-201012.nc'))
    var_raw5<-nc_open(paste0(dir_hist,'Ethiopia/timmean/',var[v],'_Amon_',mod[5],'_historical_r1i1p1f1_ethi_climate_199101-201012.nc'))
    ...
    fraw1_new<-raw1_new[3:51,3:51];fraw2_new<-raw2_new[4:52,4:52]; fraw3_new<-raw3_new[3:51,3:51]; fraw4_new<-raw4_new[3:51,3:51]
    fraw5_new<-raw5_new[3:51,3:51]
    ## bind raw
    ...
    if(v==1){
      ...
      l<-c(12,52)
      b<-seq(12,52,5)
      tit=paste0('Ethiopia Tmax in'," ",pd1[p])
      unit="0C"
    } else if(v==2){
      cols=c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026')
      l<-c(10,34)
      b<-seq(10,34,3)
      tit=paste0('Ethiopia Tmin in'," ",pd1[p]) 
      unit="0C"
    } else {
      cols=c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026')
      l<-c(12,28)
      b<-seq(12,28,2)
      tit=paste0('Ethiopia Srad in'," ",pd1[p]) 
      unit="MJm-2"
    }
    ...
    ggsave(plot = gg, 
           filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/climate/bias_cor/ethi_',var[v],'_climate_',pd1[p],'.png'), 
