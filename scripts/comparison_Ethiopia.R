...
for (k in 1:2){ ## periods
  for (i in 1:3){ ## varibles
    for (j in 1:4){ ## scenarios
      ...
      data<-brick(var_mod_all2, xmn=35.125, xmx=47.375, ymn=3.125, ymx=15.125, 
      ...
      if(i==1){
        x<-c(10,50)
        y<-seq(10,50,5)
        tit=paste0('Ethiopia Tmax annual average'," ",pd[k]," ",sc1[j])
        unit="0C"
      } else if(i==2){
        x<-c(1,33)
        y<-seq(1,33,4)
        tit=paste0('Ethiopia Tmin annual average'," ",pd[k]," ",sc1[j])
        unit="0C"
      } else {
        x<-c(16,24)
        y<-seq(16,24,1)
        tit=paste0('Ethiopia Srad annual average'," ",pd[k]," ",sc1[j])
        unit="MJm-2"
      }
      ...
      ggsave(plot = gg, 
             filename = paste0('//dapadfs/workspace_cluster_12/AVISA/data_for_dssat_eaf/plots/others/ggplot/et_',varbl[i],'_raw_bc_',tim[k],'_',sc1[j],'.png'), 
